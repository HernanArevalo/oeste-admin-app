...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const productIds = [
      ...new Set(saleInput.items.map((item) => item.product_id)),
    ];
    ...
    const { data: products, error: productsError } = await supabase
      ...
      .in("id", productIds)
      .returns<
        Pick<Product, "id" | "name" | "price" | "stock" | "is_active">[]
      >();
    ...
    // Validamos únicamente existencia y estado del producto (se removió la verificación de stock)
    ...
    // 2. Insertamos los items de la venta
    ...
    // 3. Descontamos stock únicamente si el producto cuenta con la cantidad suficiente
    for (const item of saleInput.items) {
      ...
      // Si el stock es insuficiente o cero, no se modifica y la venta continua sin lanzar ningún error.
    }
    ...
    if (updatedStockDeductions.length > 0) {
      await Promise.all(
        updatedStockDeductions.map(async (item) => {
          try {
            // Se usa await para ejecutar el RPC y capturar cualquier error
            ...
            // Fallback en caso de que no exista la función RPC en Supabase
