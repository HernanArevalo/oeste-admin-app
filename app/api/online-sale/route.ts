...
const onlineSalesSchema = z.union([onlineSaleSchema, z.array(onlineSaleSchema).min(1)])

type OnlineSaleInput = z.infer<typeof onlineSaleSchema>
type OnlineSaleProductInput = z.infer<typeof onlineSaleProductSchema>
type ProductLookup = Pick<Product, 'id' | 'name' | 'variant' | 'price' | 'stock' | 'is_active'>
...
function findProductByTitle(products: ProductLookup[], item: OnlineSaleProductInput) {
...
function isPaid(paymentStatus: string) {
  return !['pendiente', 'pending', 'no pagado', 'unpaid'].includes(normalize(paymentStatus))
}
...
export async function POST(request: NextRequest) {
  ...
  if (!validation.success) {
    return jsonError('Invalid online sale data', 400, validation.error.flatten())
  }

  const salesInput = Array.isArray(validation.data) ? validation.data : [validation.data]
  ...
  try {
    ...
    for (const saleInput of salesInput) {
      const { data: paymentMethod, error: paymentMethodError } = await supabase
        ...
        .select('id, name')
        .ilike('name', saleInput.order.paymentMethod)
        .maybeSingle<{ id: string; name: string }>()

      if (paymentMethodError) throw paymentMethodError
      if (!paymentMethod) {
        return jsonError(`Payment method ${saleInput.order.paymentMethod} not found`, 404)
      }

      const matchedItems = saleInput.order.products.map((item) => ({
        input: item,
        product: findProductByTitle(products ?? [], item),
      }))
      const invalidItems = matchedItems.flatMap(({ input, product }) => {
        if (!product) return [`Product ${input.title} was not found`]
        if (product.stock < input.quantity) {
          return [`Product ${getProductTitle(product)} has insufficient stock (${product.stock})`]
        }
        return []
      })
      ...
      const discount = Math.max(saleInput.order.subtotal + shippingCost - saleInput.order.total, 0)
